...
export function Tasks(){
    ...
    const [activeTask,setActiveTask] = useState(userTasks[0]);

    function selectActiveTask(task:userTasksIntf) {
        setActiveTask(task);
    }
    return (
        <div id='dashboard' className='flex  gap-6 bg-white overflow-y-auto  w-11/12 max-w-[950px] h-full p-[28px] border border-gray-300 shadow-lg max-md:p-[12px] max-md:w-[95%] max-md:flex-col-reverse'>
            <section id="My-Tasks" className="flex flex-col gap-5 w-5/12 rounded-2xl shadow-xl p-5 max-md:w-full">
            ...
            <section className="w-7/12 flex flex-col gap-5 rounded-2xl shadow-xl p-5 max-md:w-full relative">
            ...
            <p className='flex gap-2 absolute bottom-3 right-3'>
                ...
                <OneIconBtn><MdDelete className='text-2xl max-md:text-xl'/></OneIconBtn>
            </p>
            </section>
        </div>
    );
